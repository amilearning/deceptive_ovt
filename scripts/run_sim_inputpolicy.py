...
def main(args=None):
    ...
    predictor_class = InputPolicyPredictor  # Either None or one of trajectory_predictor classes
    ...
    if predictor_class is not None:
        ...
        elif predictor_class.__name__ == "InputPolicyPredictor":
            predictor = InputPolicyPredictor(vehicle_model= tar_dynamics_simulator.model, N=N, track=track_obj, policy_name=gp_model_name, use_GPU=use_GPU, M=M, cov_factor=np.sqrt(2))            
        else:

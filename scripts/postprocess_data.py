...
def main(args=None):
        ...
        if exists:
            for id, a in enumerate(names):
                ...
                if os.path.exists(name):
                    ...
                    # if metrics.lateral_error
                    if len(metrics.lateral_error) ==0 :
                        continue
    ...
    plt.xlabel("Lateral error (m)", fontsize=font_size)
    ...
    # plt.legend(names_for_legend)
    plt.ylim((0, 50))
    plt.xlim((-0.3, 0.3))

    plt.subplot(1, 2, 2)
    plt.tick_params(axis='both', labelsize=font_size)
    plt.xlabel("Longitudinal error (m)", fontsize=font_size)
    plt.ylabel("Density", fontsize=font_size)
    for i in range(n):
    ...
    for i in range(n):
        print(names[i]+"lateral_errors mean = " + "{:.3f}".format(np.mean(lateral_errors[i])) + ", std =" + "{:.3f}".format(np.std(lateral_errors[i])))
    for i in range(n):
        print(names[i]+"longitudinal_errors mean = " + "{:.3f}".format(np.mean(longitudinal_errors[i])) + ", std =" + "{:.3f}".format(np.std(longitudinal_errors[i])))

...
def main(args=None):
    ...
    if predictor_class.__name__ == "GPPredictor" or predictor_class.__name__ == "ContThetaPolicyPredictor":
    ...
    if predictor_class is not None:
        ...
        elif predictor_class.__name__ == "ContThetaPolicyPredictor":
            predictor = ContThetaPolicyPredictor(N=N, track=track_obj, policy_name=gp_model_name, use_GPU=use_GPU, M=M, cov_factor=np.sqrt(2))            
        else:
